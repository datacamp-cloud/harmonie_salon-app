...
import { Loader2, Plus, CheckCircle2, Receipt } from 'lucide-react'
...
function Income() {
  ...
  // Quand on sélectionne une prestation, on pré-remplit le prix
  const handlePrestationChange = (prestationId) => {
    const prestation = prestations.find((p) => p.id === Number(prestationId))
    setFormData((c) => ({
      ...c,
      prestationId,
      prixApplique: prestation ? String(prestation.prix) : '',
    }))
  }
  ...
  const totalMois = useMemo(() => {
    ...
    return recettes
      .filter((r) => r.isValidated && r.date.startsWith(month))
      .reduce((t, r) => t + r.prixApplique, 0)
  }, [recettes])
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-beige-600 mt-1">
            Enregistrez les prestations realisees et leur montant encaisse.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_0.95fr] gap-6">
            ...
            <div>
              ...
              {prestationsActives.length === 0 && (
                <p className="text-xs text-amber-600 mt-1">
                  Aucune prestation active. Configurez-en dans Parametres &gt; Prestations.
                </p>
              )}
            </div>
            ...
            <label className="flex items-center gap-3 rounded-lg border border-beige-200 bg-beige-50 px-4 py-3 cursor-pointer">
              ...
              <div>
                <span className="text-sm font-medium text-beige-800">
                  Valider et comptabiliser dans la caisse
                </span>
                <p className="text-xs text-beige-500 mt-0.5">
                  Une fois validee, la recette ne peut plus etre modifiee.
                </p>
              </div>
            </label>
        ...
        <div className="bg-white rounded-xl border border-beige-200 p-6">
          <div className="flex items-center justify-between gap-3 mb-6">
            ...
            <span className="text-sm text-beige-400">{recettes.length} recette(s)</span>
          </div>
          ...
          {isLoading ? (
            ...
            <ul className="space-y-3 max-h-[42rem] overflow-y-auto pr-1">
              {[...recettes].reverse().map((recette) => (
                <li key={recette.id} className="rounded-xl border border-beige-200 bg-beige-50 p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p className="font-medium text-beige-900">{recette.prestationNom}</p>
                      <p className="text-sm text-beige-500 mt-0.5">
                        {recette.clientNom || 'Client anonyme'}
                      </p>
                    </div>
                    <div className="flex items-center gap-2">
                      {!recette.isValidated && (
                        <button
                          type="button"
                          onClick={() => validateMutation.mutate(recette.id)}
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-beige-900">
                          {recette.prixApplique.toLocaleString('fr-FR')} FCFA
                        </span>
                        <span
                  ...
                  {recette.notes && (
                    <p className="text-xs text-beige-500 mt-2">{recette.notes}</p>
                  )}

                  <div className="mt-2 text-xs text-beige-400">{recette.date}</div>
                </li>
              ))}
            </ul>
          ) : (
